...
function BlogContent() {
  ...
  useEffect(() => {
    const handlePopState = (event: PopStateEvent) => {
      ...
      if (state && state.viewMode) {
        ...
        setSelectedArticleId(state.selectedArticleId || null);
  ...
  const handleNavigate = (path: string) => {
    if (path === '/') {
      ...
      handleShowArticles();
    } else if (path === '/public/articles/README.md') {
      // 跳转到使用指南文章
      const readmeArticle = articles.find(article => article.id === 'README');
      if (readmeArticle) {
        handleSelectArticle(readmeArticle.id);
      } else {
        // 如果找不到，使用默认导航
